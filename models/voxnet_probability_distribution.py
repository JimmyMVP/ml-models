...
conv2 = conv3d(conv1, 3,28,stride=2)
...
loss = tf.reduce_mean(tf.nn.weighted_cross_entropy_with_logits(output_logits, flatten(x), flatten(x)*np.prod(RES)+1)) + lossL2

print("Loss shape: ", loss.get_shape())

#Training
optimizer = tf.train.AdamOptimizer(0.06)
...
with tf.Session() as sess:
    ...
    for i in range(EPOCHS):
        ...
        for batch in range(training_files.size//BATCH_SIZE):
            ...
            _, r_loss, r_precision, r_output = sess.run([train_step, loss, precision, output], feed_dict=feed_dict) 
            if(batch%5==0):
                print("Number of classes: ", np.unique(class_names).size)
                print("Batch step %d,  loss: %.2f, precision: %.5f, dropout: %.2f" % (batch, r_loss, r_precision, iter_dropout))
        ...
        r_loss, r_precision, r_predictions, r_equals, r_recall = sess.run([loss, precision, output, equals, recall], feed_dict=feed_dict)
        l()
